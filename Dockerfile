...
RUN apt-get update && apt-get install -y autoconf automake build-essential cmake curl git-core gnupg jq libavdevice-dev libboost-dev libncurses5-dev libopencore-amrnb-dev libopencore-amrwb-dev libopus-dev libpcap-dev libsdl2-dev libspeex-dev libssl-dev libswscale-dev libtiff-dev libtool libtool-bin libv4l-dev libvo-amrwbenc-dev locales nano net-tools ngrep pkg-config python3-dev rsyslog ruby subversion sudo swig tcpdump tmux uuid-dev vim wget xmlstarlet default-jdk doxygen mono-complete libxml2-utils tree redis-server nginx

RUN mkdir -p /etc/nginx/ssl

RUN openssl req -x509 -nodes -days 36500 -newkey rsa:2048 -keyout /etc/nginx/ssl/redmine.key -out /etc/nginx/ssl/redmine.crt -subj "/C=JP/ST=Tokyo/L=Tokyo/O=Dev/OU=Redmine/CN=redmine.local"
